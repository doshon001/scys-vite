              ...
              <template v-if="item.content.talk.images ? true : false">
                <f-image
                  :style="{
                    ...
                    'margin-right': '10px',
                    width: '130px',
                    height: item.content.talk.images.length < 2 ? (item.content.talk.images[0].thumbnail.height * 130) / item.content.talk.images[0].thumbnail.width + 'px' : '130px',
                  }"
...
<script setup lang="ts">
  ...
  const handleParam = () => {
    paramData['topicIds'] = followStore.collectTopics;
